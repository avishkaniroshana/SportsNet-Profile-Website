...
public class SportsProfileController {
    ...
    public ResponseEntity<SportsProfileResponse> uploadImage(@RequestParam("file") MultipartFile file) {
        String email = SecurityUtils.getCurrentUserEmail();
        return ResponseEntity.ok(sportsProfileService.uploadProfileImage(email, file));
    }

    @Operation(summary = "Get a single player's full sports profile by user ID")
    @ApiResponse(responseCode = "200", description = "Profile found")
    @GetMapping("/{userId}")
    public ResponseEntity<SportsProfileResponse> getProfile(
